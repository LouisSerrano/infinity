...
from infinity.data.dataset import get_dataset
...
def Infer_test(device, models, hparams, data, coef_norm=None):
    ...
    for n, out in enumerate(outs):
        ...
        if coef_norm is not None:
            outs[n][data.surf, :2] = (
                -torch.tensor(coef_norm[2][None, :2])
                * torch.ones_like(out[data.surf, :2])
                / (torch.tensor(coef_norm[3][None, :2]) + 1e-8)
            )
            outs[n][data.surf, 3] = (
                -torch.tensor(coef_norm[2][3])
                * torch.ones_like(out[data.surf, 3])
...
def Results_test(
    ...
    Args:
        ...
        models (torch_geometric.nn.Module): List of models_saved to predict with. It is a list of a list of different training of the same model.
            For example, it can be [model_MLP, model_GraphSAGE] where model_MLP is itself a list of the form [MLP_1, MLP_2].
        hparams (dict): Dictionnary of hyperparameters of the models_saved.
    ...
    test_dataset_vtk = get_dataset(test_dataset, sample=None, coef_norm=coef_norm)
    ...
    for i in range(len(models[0])):
        ...
        for j, data in enumerate(tqdm(test_loader)):
            ...
            times.append(tim)
            intern = pv.read(
                "Dataset/" + test_dataset[j] + "/" + test_dataset[j] + "_internal.vtu"
            )
            aerofoil = pv.read(
                "Dataset/" + test_dataset[j] + "/" + test_dataset[j] + "_aerofoil.vtp"
            )
            ...
            intern, aerofoil = Airfoil_test(
                intern, aerofoil, outs, coef_norm, data.surf
            )
            ...
            for n, out in enumerate(outs):
                ...
                loss_vol_var = criterion(
                    out[~data.surf, :], data.y[~data.surf, :]
                ).mean(dim=0)
                ...
                b.append(
                    np.array(
                        metrics_NACA.boundary_layer(
                            airfoil_mean, internal_mean, aero_name, x
                        )
                    )
                )

...
from infinity.utils.naca_generator import camber_line
...
def compare_surface_coefs(coefs1, coefs2, extrado=True, path=None):
    ...
    if extrado:
        ...
        ax[0].scatter(
            ycp2[:n_extrado2], c_p2[:n_extrado2], color="y", label="Extrado Target"
        )
        ax[0].scatter(
            ycp2[n_extrado2:],
            c_p2[n_extrado2:],
        ...
        ax[1].scatter(
            ycl2[:n_extrado2], c_f2[:n_extrado2], color="y", label="Extrado Target"
        )
        ax[1].scatter(
            ycl2[n_extrado2:],
            c_f2[n_extrado2:],
...
def plot_coef_convergence(path, params):
    datas = dict()
    datas["c_d"] = np.loadtxt(path + "postProcessing/forceCoeffs1/0/coefficient.dat")[
        :, 1
    ]
    datas["c_l"] = np.loadtxt(path + "postProcessing/forceCoeffs1/0/coefficient.dat")[
        :, 3
    ]
